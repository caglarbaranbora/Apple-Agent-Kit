...
- 2026-08-07 — `xcode` gains test plans, code coverage, and project localization — 4 Knowledge Contracts. This closes two hand-offs that had been pointing here since before either receiving Contract existed: `testing` had deferred Test Plans and code coverage to `xcode`, and `localization` had deferred adding a language and the `.xcloc`/XLIFF round trip. The `localization` ↔ `xcode` boundary the domain map recorded as **unresolved** is now resolved. Several of the rules exist because the failure is silent: an export taken with Use Compiler to Extract Swift Strings off succeeds and simply omits every Swift string; a test excluded from a plan reports no status at all rather than a skip; a coverage percentage counts lines reached only by tests marked as expected failures; and a resource left unchecked in the add-a-language sheet is never offered for translation and never warned about. `xcodebuild` CLI, CI signing automation, SwiftPM build configuration, and macOS signing/notarization are **Excluded**; Xcode Cloud is **Deferred**.
