...
**2026-09-10 note:** `docs/adr/` was introduced (see ../docs/specifications/adr-spec.md), which reads as reopening this decision at first glance. It is not: this decision rejected `docs/adr/` as a *general* decision-record system duplicating what `rfcs/` already does — one-shot, approve-then-apply architecture proposals. What `docs/adr/` actually holds is narrower and shaped differently: a per-domain, append-only log of an already-effective scope (extracted out of `docs/architecture/domain-map.md`'s own growing narrative paragraph, not a new proposal-and-approval flow). `rfcs/` remains the only place an architecture *change* is proposed and approved; `docs/adr/` never supersedes that. See `adr-spec.md`'s "Relationship to `rfcs/`" section for the full distinction.
